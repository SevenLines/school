...
public class StudyActivityService {
    ...
    public StudyActivity update(StudyActivity studyActivity) {
        StudyActivity activityFromDB = studyActivityRepository.findById(studyActivity.getId())
                .orElseThrow(() -> new NoSuchElementException("Мероприятие с таким id не найдено!"));
        activityFromDB.setName(studyActivity.getName())
    ...
    public StudyActivity create(StudyActivity studyActivity) throws StudyActivityAlreadyExistException {
        StudyActivity existingStudyActivity =
                studyActivityRepository.findExistingInDB(studyActivity.getName(), studyActivity.getStartDate());
    ...
    public ActivityHistory register(ActivityAppointmentDto registerDto) {
        ...
        ActivityHistory existedHistoryRecord =
                activityHistoryRepository.findExistingInDB(ActivityHistoryRecordType.REGISTERED.getName(),
                        registerDto.getActivityId(),
                        registerDto.getSchoolchildId());
        if (existedHistoryRecord != null) {
            throw new IllegalArgumentException("В базе данных уже существует отметка о том, что школьник с id="
                    + registerDto.getSchoolchildId()
                    + " зарегистрировался на мероприятие с id="
                    + registerDto.getActivityId());
        }
        ActivityHistory activityHistory = new ActivityHistory().setActivity(studyActivity)
    ...
    public ActivityHistory appointment(ActivityAppointmentDto appointmentDto) {
        ...
        StudyActivity attendedActivity = studyActivityRepository.findById(appointmentDto.getActivityId())
                .orElseThrow(() -> new NoSuchElementException("Мероприятие с таким activityId не найдено"));

        logger.info("StudyActivityService StudyActivity attendedActivity: " + attendedActivity);
        Schoolchild schoolchild = schoolchildRepository.findById(appointmentDto.getSchoolchildId())
                .orElseThrow(() -> new NoSuchElementException("Школьник с таким schoolchildId не найден"));

        logger.info("StudyActivityService schoolchild: " + schoolchild);
        final ActivityHistory attendedFormDb =
                activityHistoryRepository.checkExist(appointmentDto.getSchoolchildId(), appointmentDto.getActivityId());

        if (attendedFormDb != null) {
            throw new IllegalArgumentException(String.format(
                    "В базе данных уже существует отметка о том, что школьник с id=%s посетил мероприятие с id=%s",
                    appointmentDto.getSchoolchildId(),
                    appointmentDto.getActivityId()));
        }
        final ActivityHistory attendHistoryRecord = new ActivityHistory().setActivity(attendedActivity)
                .setSchoolchild(schoolchild)
    ...
    public void deleteAppointment(ActivityAppointmentDto deleteAppointmentDto) {
        ...
        if (activityHistory == null) {
            throw new IllegalArgumentException(String.format(
                    "В базе данных нет отметки о посещении мероприятия " + "с id=%s; для пользователя с id=%s",
                    deleteAppointmentDto.getActivityId(),
                    deleteAppointmentDto.getSchoolchildId()));
        }
